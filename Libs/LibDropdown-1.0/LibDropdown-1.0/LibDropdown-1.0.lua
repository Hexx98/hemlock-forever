...
local MINOR = 4 -- 3: modern-client fixes (tooltip backdrop, color swatches); 4: pooled slider callback
...
-- Modern clients draw GameTooltip with NineSlice and it has no backdrop to copy,
-- so fall back to the classic tooltip look.
...
local function InitializeFrame(frame)
	...
	if backdrop then
		...
		frame:SetBackdropColor(0.09, 0.09, 0.19, 0.95)
...
local function ReleaseSlider(slider)
	...
	slider.ValueChanged = nil -- a pooled slider must not write into its next owner
	slider:Hide()
...
do
	...
	local function highlightSwatch(self)
		self.tex:SetColorTexture(NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b)
	end

	local function unhighlightSwatch(self)
		self.tex:SetColorTexture(HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b)
	end
...
do
	...
	do
		...
		local function showSlider(frame)
			...
			slider.ValueChanged = nil -- configure silently; OnValueChanged can fire below
			...
			slider:Show()
			refresh(frame:GetParent())
			slider.ValueChanged = sliderValueChanged -- only now, showing the stored value
		end
