...
frame:SetScript("OnShow", function(frame)
	...
	description:SetText("Minimalistic addon to automate poison buying and creation, for WoW Forever")

	local function newCheckbox(name, label, description, onClick)
		-- InterfaceOptionsCheckButtonTemplate may not exist on this client
		local ok, check = pcall(CreateFrame, "CheckButton", "HemlockCheckBox" .. name, frame, "InterfaceOptionsCheckButtonTemplate")
		if not ok then
			check = CreateFrame("CheckButton", "HemlockCheckBox" .. name .. "Alt", frame, "UICheckButtonTemplate")
			_G["HemlockCheckBox" .. name] = check
		end
		...
		check.label = _G[check:GetName() .. "Text"] or check.Text or check.text
		if not check.label then
